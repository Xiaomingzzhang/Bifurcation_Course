...
```
```{prf:example}
假设 $f(x,t)$ 在区域 $U$ 上关于 $x$ 具有一阶连续且有界的偏导数, 那么 $f$ 是关于 $x$ 局部 Lipschitz 的. 实际上,
设 $K\subset U$ 是任意有界闭集. 由于 $D_x f(x,t)$ 有界, 即存在常数 $M>0$, 使得
$$
\|D_x f(x,t)\|\le M, \qquad \forall (x,t)\in K.
$$

任取 $(x_1,t),(x_2,t)\in K$, 且固定 $t$, 考虑连接 $x_1,x_2$ 的线段
$$
\gamma(s)=x_2+s(x_1-x_2),\qquad s\in[0,1].
$$
由微积分基本定理, 有
$$
f(x_1,t)-f(x_2,t)
= \int_0^1 \frac{d}{ds} f(\gamma(s),t)\, ds
= \int_0^1 D_x f(\gamma(s),t)\,(x_1-x_2)\, ds.
$$

于是
$$
\|f(x_1,t)-f(x_2,t)\|
\le \int_0^1 \|D_x f(\gamma(s),t)\|\,\|x_1-x_2\|\, ds
\le M\|x_1-x_2\|.
$$

因此在 $K$ 上, $f$ 关于 $x$ 满足 Lipschitz 条件, Lipschitz 常数可以取为 $M$. 由于 $K$ 是任意的有界闭集, 故 $f$ 关于 $x$ 是局部 Lipschitz 连续的.
```
...
## 混沌的含义
动力系统最广为人知的概念之一就是混沌. 我们在粗略介绍一下混沌, 并给出混沌的最简单的一个例子. 广为人知的蝴蝶效应, "一只蝴蝶在巴西轻拍翅膀, 可以导致一个月后德克萨斯州的一场龙卷风", 实际上也可以用数学语言严格描述. 这个描述称为系统对初值的敏感依赖性.

```{prf:definition}
假设映射 $g:X\mapsto X$ 是连续的, 如果任给 $\epsilon>0$, 存在不依赖于 $\epsilon$ 的常数 $\delta>0$, 以及正整数 $n$ (可能依赖于 $\epsilon$) 使得当 $0<d(x,y)<\epsilon$ 时, $d(g^{n}(x),g^{n}(y))>\delta$, 则称映射 $g$ 在 $X$ 上具有对初值的敏感依赖性. 其中 $x,y\in X$ 是任意的两点, $d(\cdot,\cdot)$ 是空间 $X$ 的距离.
```

上述定义即称, 只要初始位置有微小的偏差 ($\epsilon$), 那么经过迭代多次以后, 必然有某一次, 轨道会发生大的偏离 ($\delta$). 

下面我们给出混沌的经典的 Dave 定义:
```{prf:definition}
映射 $g:X\mapsto X$ 称作是混沌的, 如果下列条件满足:
- $g$ 在 $X$ 上具有对初值的敏感依赖性;
- $g$ 的周期轨道在 $X$ 上稠密;
- 存在 $g$ 的一条轨道, 该轨道在 $X$ 上稠密.
```
